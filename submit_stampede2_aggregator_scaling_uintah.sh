...
# generated particles
# Particles in the uintah sim are 7 double attribs + 3 float positions
particle_size=68
# At the final timestep, the uintah has 46M particles
total_particles=46149100
...
for i in `seq 0 $num_tests`; do
    ...
    export OUTPUT=agg-scaling-$i-uintah-${num_nodes}${PARTITION}
    if [ -n "$DUMP_RAW" ]; then
